...
- 竞争条件的各种例子
  ...
  - 幻读(phantom reads)
    ...
    注：可重复读为何可以保证同一个事务内读取具体行的结果是一致的，却无法保证一定目标范围内的数据是一致的？理解这个问题需要清楚锁加的对象是什么，读写某条记录，申请的锁是可能是某个具体对象（即具体的数据行），也可能是锁一定范围（如果事务1获取范围锁，事务2操作目标范围，发现目标范围被锁，陷入阻塞），也可能锁整表
